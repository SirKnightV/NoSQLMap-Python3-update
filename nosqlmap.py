...
import sys
from pruebanosqlx import nsmcouch
from pruebanosqlx import nsmmongo
import string
import random
import os
import time
...
def postApps():
	...
	try:
		...
		if appRespCode == 200:
			...
			normLength = int(len(urllib.request.urlopen(req).read()))
	...
	if appUp == True:
		...
		body = urllib.parse.urlencode(postData)
		req = urllib.request.Request(appURL,body, requestHeaders)
		randLength = int(len(urllib.request.urlopen(req).read()))
		...
		errorCheck = errorTest(str(urllib.request.urlopen(req).read()),testNum)

		if errorCheck == False:
			injLen = int(len(urllib.request.urlopen(req).read()))
...
def getApps():
	...
	if appUp == True:
		...
		if doTimeAttack in yes_tag:
			...
			start = time.time()
			strTimeInj = urllib.request.urlopen(req)
			...
			start = time.time()
			intTimeInj = urllib.request.urlopen(req)
...
def getDBInfo():
	...
	baseLen = int(len(urllib.request.urlopen(req).read()))
	...
	while gotNameLen == False:
		...
		lenUri = int(len(urllib.request.urlopen(req).read()))
